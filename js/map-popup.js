function generatePopupText(item, popupColor) {
    ...
    for (const [key, value] of Object.entries(item.tags || {})) {
        if (key === "name" || key === "access" || key === "capacity" || key === "fee" || key === "parking" || key === "operator") {
...
function extendPopupLabelList(itemId) {
    ...
    const popupListRowDivs = popupDiv.querySelectorAll('.map-popup-list-row-extendable');

    if(isExtended) {
        popupListRowDivs.forEach((row) => {row.classList.add('hidden-force')});
    }
    else {
        popupListRowDivs.forEach((row) => {row.classList.remove('hidden-force')});
    }
}
