...
let currentRadius = 1.0; // Initial search radius in km
...
function main() {
  if (navigator.geolocation) {
    ...
    showMapWithDummyLocation();
  }
}

function showMapWithDummyLocation() {
  // Use a fixed location
  userLocation = { lat: 49.0069, lng: 8.4037 }; // Karlsruhe, Germany
...
function initMap(location) {
  ...
  map.on("click", () => {
    document.querySelectorAll("[data-item-id]").forEach((elem) => {
      elem.classList.remove("parking-item-selected");
    });
  });
}
...
function extractCoordinatesFromRawParkingData() {
  parkingData.forEach((item) => {
    ...
    if (item.type === "node") {
      ...
      const memberLatLngs = item.members
        .filter(
          (member) =>
            member.type === "node" &&
        ...
        .map((member) => [member.lat, member.lon]);
  ...
  parkingData.forEach((item) => {
    ...
    if (item.type === "node") {
      ...
      if (memberLatLngs.length > 0) {
        const popupText = generatePopupText(item, color);
        const polygon = L.polygon(memberLatLngs, {
          color: color,
          fillOpacity: 0.5,
        });
        polygon.addTo(map).bindPopup(popupText);
...
function shouldShowParking(tags) {
  ...
  const paidCustomersChecked = document.getElementById(
    "filter-paid-customers"
  ).checked;
  const paidChecked = document.getElementById("filter-paid").checked;
  const permissiveChecked =
    document.getElementById("filter-permissive").checked;
  ...
  if (tags.access === undefined && tags.fee === "no" && freeChecked)
    return true;
  if (
    tags.access === "customers" &&
    (tags.fee === "no" || tags.fee === undefined) &&
    customersChecked
  )
    return true;
  if (tags.access === "customers" && tags.fee === "yes" && paidCustomersChecked)
    return true;
  if (tags.fee === "yes" && paidChecked && tags.access !== "customers")
    return true;
